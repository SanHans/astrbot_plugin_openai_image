...
- 插件自然语言触发：先使用当前会话正在使用的 AstrBot provider，按插件自己的“自然语言润色模板”输出 `prompt + reply`；其中 `prompt` 轻度润色但不改变本意，`reply` 参考当前人格口吻
...
- `natural_language_fallback_reply`：润色失败时自然语言回复的兜底文本
...
- 指令路径默认会先发一条“正在生成图片，请稍等……”，自然语言路径不会；自然语言会在最终结果里附带一条符合当前人格口吻的简短回复
- 如果自然语言触发已处理消息，默认会调用 `event.stop_event()`，避免同一条消息继续流向后续 LLM 处理造成重复回复
- `gpt-image-2` 官方当前不支持透明背景；如果你使用的是兼容服务并支持其他模型，可自行切换
- 如果你使用的是 `wecom_ai_bot`（企业微信智能机器人）渠道，插件会避免先发送进度文本，尽量把人格化回复和图片合并为一次最终回复；如果仍有图片不回传，日志里会明确提示配置 `msg_push_webhook_url`
